...
const EscalationCard: React.FC<EscalationCardProps> = ({
    ...
    const priorityConfig = {
        ...
        low: { color: 'text-slate-600', bg: 'bg-slate-50', border: 'border-slate-200', icon: '💬', wait: '< 15 min' }
    };
        ...
        return (
            <div className="bg-gradient-to-r from-primary/5 to-primary/10 rounded-2xl p-6 border border-primary/20 animate-pulse">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h4 className="font-semibold text-slate-800">Connecting to Support...</h4>
                        <p className="text-sm text-slate-500">Please wait while we find an available agent</p>
                    </div>
                </div>
        ...
        <div className={`rounded-2xl border ${config.border} ${config.bg} overflow-hidden animate-in slide-in-from-bottom-2 duration-300`}>
            ...
            <div className="px-5 py-4 border-b border-slate-100/50 bg-white/40">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h4 className="font-bold text-slate-800">Human Support</h4>
                    ...
                    {ticket?.id && (
                        <div className="text-right">
                            ...
                            <p className="font-mono font-bold text-slate-700">#{ticket.id}</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="px-5 py-4 bg-white/30">
                <div className="flex items-center gap-3 mb-3">
                    {ticket?.status === 'pending' ? (
                        <>
                            ...
                            <span className="text-sm text-slate-600">Waiting for an agent...</span>
                        </>
                        ...
                        <>
                            ...
                            <span className="text-sm text-slate-600">Creating ticket...</span>
                        </>
                    )}
                </div>
                ...
                {ticket?.status === 'pending' && (
                    <div className="bg-white rounded-xl p-3 border border-slate-100">
                        <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="font-semibold text-slate-800">{config.wait}</span>
                        </div>
                ...
                {showAgentTyping && ticket?.status === 'pending' && (
                    <div className="mt-3 flex items-center gap-2 text-sm text-slate-500 animate-in fade-in duration-500">
            ...
            {/* Tips */}
            <div className="px-5 py-3 bg-white/20 text-xs text-slate-500">
