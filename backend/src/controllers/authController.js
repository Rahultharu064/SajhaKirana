...
export const registerUser = async (req, res, next) => {
    try {
        ...
        const token = jwt.sign({ userId: newUser.id, role: newUser.role ?? "user" }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
        res.status(201).json({
            success: true,
...
export const loginUser = async (req, res, next) => {
    try {
        ...
        const token = jwt.sign({
            userId: user.id,
            role: user.role ?? "customer" // fallback since role column may not exist yet
        }, JWT_SECRET, {
        ...
        res.status(200).json({
            ...
            user: {
                ...
                name: user.name,
                email: user.email,
                phone: user.phone ?? null, // fallback since phone column may not exist yet
                address: user.address ?? null, // fallback since address column may not exist yet
                role: user.role ?? "customer" // fallback since role column may not exist yet
            },
            token
        });
    ...
    try {
        ...
        res.status(200).json({
            ...
            user: {
                ...
                role: updatedUser.role ?? "customer", // fallback since role column may not exist yet
...
export const sendVerificationEmail = async (req, res, next) => {
    try {
        ...
        const verificationToken = jwt.sign({ userId: user.id, type: 'email_verification' }, JWT_SECRET, { expiresIn: '24h' });
        // Send verification email
        try {
...
export const createAdmin = async (req, res, next) => {
    try {
        ...
        const token = jwt.sign({ userId: newAdmin.id, role: newAdmin.role }, JWT_SECRET, { expiresIn: '24h' } // Longer expiration for initial admin setup
        );
        res.status(201).json({
            success: true,
