...
export const chatbotController = {
    ...
    async getRecommendations(req, res) {
        try {
            ...
            const recommendations = await recommendationService.recommendProducts(userId, 10);
            res.json({
                success: true,
                recommendations,
            });
    ...
    // Get budget products
    async getBudgetProducts(req, res) {
        try {
            const maxPrice = parseInt(req.query.maxPrice) || 500;
            const limit = parseInt(req.query.limit) || 10;
            const products = await recommendationService.getBudgetProducts(maxPrice, limit);
            res.json({
                success: true,
                products,
            });
    ...
    async getSuggestedQuestions(req, res) {
        try {
            ...
            const suggestions = await conversationMemoryService.getSuggestedQuestions(sessionId);
            res.json({
                success: true,
                suggestions,
            });
