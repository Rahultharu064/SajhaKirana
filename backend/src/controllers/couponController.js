...
export const createCoupon = async (req, res, next) => {
    try {
        const { code, description, discountType, discountValue, minOrderValue = 0, maxDiscount, usageLimit = 0, expiryDate, isActive = true } = req.body;
...
export const getValidCoupons = async (req, res, next) => {
    try {
        const { orderValue } = req.query;
        const minOrderValue = orderValue ? parseFloat(orderValue) : 0;
