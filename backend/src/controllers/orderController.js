...
export const getAllOrders = async (req, res, next) => {
    try {
        ...
        res.status(200).json({ success: true, data: orders, pagination: { page: parseInt(page), limit: parseInt(limit), total } });
    }
    catch (error) {
        next(error);
    }
...
export const updateOrderStatus = async (req, res, next) => {
    try {
        ...
        const changedBy = req.user?.id; // Admin who made the change
        ...
        try {
            ...
            await sendOrderStatusEmail(order.user.email, order.user.name, id, status, updatedOrder.otp || undefined);
        }
        catch (emailError) {
