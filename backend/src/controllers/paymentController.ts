...
// Helper for eSewa
export const processEsewaInitiation = async (orderId: number, baseUrl: string) => {
...
// Helper for Khalti
export const processKhaltiInitiation = async (orderId: number, baseUrl: string, user: { name: string, email: string, phone: string }) => {
  const order = await prismaClient.order.findUnique({
    where: { id: orderId },
    include: { payments: true }
  });
...
export const verifyKhaltiPayment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { pidx, purchase_order_id } = req.query as any;
