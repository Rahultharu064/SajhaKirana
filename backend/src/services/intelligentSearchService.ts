...
export interface SearchResult {
    ...
    relevanceScore: number; // Pure semantic score
    metadata: any;
}
...
export class IntelligentSearchService {
     ...
     */
    async search(query: string, options: { userId?: number; sessionId?: string; limit?: number } = {}) {
        ...
        for (const res of semanticResults) {
            ...
            if (dbProduct.stock > 0) rankingScore += 0.2;
            ...
            if (daysSinceCreated < 7) rankingScore += 0.05;
