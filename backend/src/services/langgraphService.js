...
const ChatStateAnnotation = Annotation.Root({
    ...
    parsedCartItems: Annotation({
        reducer: (x, y) => y ?? x,
        default: () => null,
    }),
    isVoiceAuthenticated: Annotation({
        reducer: (x, y) => y ?? x,
        default: () => false,
    }),
...
export class LangGraphChatbot {
    ...
    initializeGraph() {
        ...
        // Define workflow
        workflow.addEdge(START, 'extract_query');
        workflow.addEdge('extract_query', 'classify_intent');
        workflow.addEdge('classify_intent', 'extract_details');
        workflow.addEdge('extract_details', 'verify_security');
        workflow.addEdge('verify_security', 'process_actions');
        workflow.addEdge('process_actions', 'extract_preferences');
        workflow.addEdge('extract_preferences', 'retrieve_context');
        workflow.addEdge('retrieve_context', 'enrich_user_context');
        workflow.addEdge('enrich_user_context', 'get_recommendations');
        workflow.addEdge('get_recommendations', 'generate_response');
        workflow.addEdge('generate_response', 'generate_suggestions');
        workflow.addEdge('generate_suggestions', END);
        this.graph = workflow.compile();
    }
    // Step 1: Extract query
    async extractQuery(state) {
    ...
    async extractShoppingDetails(state) {
    ...
    async verifyVoiceSecurity(state) {
        const query = state.query.toLowerCase();
    ...
    async handleOrderActions(state) {
        if (!state.userId) {
            return {};
        }
        try {
    ...
    async extractPreferences(state) {
        const query = state.query.toLowerCase();
    ...
    async retrieveContext(state) {
        try {
            ...
            const retrievedDocs = await embeddingService.searchSimilar(state.query, 10, filter.must.length > 0 ? filter : undefined);
            let context = '';
    ...
    async getRecommendations(state) {
        try {
            ...
            if (state.intent === 'recommendation' && uid) {
                ...
                recommendations = await recommendationService.recommendProducts(uid, 5);
            }
                ...
                // Budget products
                recommendations = await recommendationService.getBudgetProducts(state.priceFilter.max, 8);
            }
...
- If voice authenticated, mention that secure actions (like checkout or clear cart) are now available.`;
    ...
    async generateSuggestions(state) {
        try {
            ...
            if (state.sessionId) {
                const context = await conversationMemoryService.getContext(state.sessionId, state.userId);
                if (context) {
                    context.currentIntent = state.intent;
                    context.isVoiceAuthenticated = state.isVoiceAuthenticated;
                    context.suggestedProducts = state.recommendations.map((r) => r.metadata?.productId || r.id);
    ...
    async chat(messages, userId, sessionId) {
        try {
            ...
            if (messages && messages.length > 0) {
                ...
                if (lastMessage) {
                    await conversationMemoryService.addMessage(sid, lastMessage.role, lastMessage.content, uid);
                }
            }
            await conversationMemoryService.addMessage(sid, 'assistant', result.response, uid);
            return {
